...
interface AlertModalProps {
  ...
  alertData?: {
    total?: number;
    recent?: Array<{ descripcion?: string; monto?: number; fecha?: string }>;
    insight?: string;
  } | null;
  title?: string;
}

export default function AlertModal({ onClose, alertData, title = "Gasto Hormiga" }: AlertModalProps) {
  const recent = alertData?.recent?.length
    ? alertData.recent
    : [
        { descripcion: "Tinto diario (x7)", monto: 21000 },
        { descripcion: "Empanadas y Pan", monto: 35000 },
        { descripcion: "Uber cortos (x3)", monto: 45000 },
        { descripcion: "Maquina Snacks", monto: 49000 },
      ];
  const total = alertData?.total ?? recent.reduce((acc, item) => acc + (item.monto || 0), 0);
      ...
      <div className="bg-[#1a2a5e] border-2 border-[#F36E53]/40 w-full max-w-sm rounded-[32px] p-6 shadow-[0_20px_60px_rgba(243,110,83,0.25)] relative overflow-hidden animate-in slide-in-from-bottom-10 zoom-in-95 duration-400">
        ...
        <div className="mb-6 relative z-10">
          <div className="text-[42px] font-black text-white tracking-tighter drop-shadow-md">
            ${total.toLocaleString()} <span className="text-xl text-white/50 font-medium">COP</span>
          </div>
          <p className="text-white/70 text-sm mt-1">Acumulado reciente</p>
        </div>
            ...
            {recent.map((item, i) => (
              <li key={i} className="flex justify-between items-center text-sm">
                <div className="flex items-center gap-2">
                  <span className="bg-white/10 w-8 h-8 rounded-full flex items-center justify-center text-base">!</span>
                  <span className="text-white font-medium">{item.descripcion || "Movimiento"}</span>
                </div>
                <span className="text-white/80 font-bold">${(item.monto || 0).toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        {alertData?.insight && (
          <p className="text-white/70 text-xs leading-relaxed mb-4 relative z-10">{alertData.insight}</p>
        )}
